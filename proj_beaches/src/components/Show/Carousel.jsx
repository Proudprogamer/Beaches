...
import { getAuth, onAuthStateChanged, signInWithPopup, GoogleAuthProvider } from 'firebase/auth';  // Firebase imports
...
const CarouselWithDetails = ({ name, beachLocation, beachId }) => {
    ...
    const [loginModalIsOpen, setLoginModalIsOpen] = useState(false); // State for login modal
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            setLoginModalIsOpen(false); // Close login modal on success
        } catch (error) {
    ...
    return (
        <div className="w-full mx-auto p-4 md:p-8 rounded-3xl shadow-2xl">
            ...
            <Modal isOpen={reviewModalIsOpen} onRequestClose={() => setReviewModalIsOpen(false)} style={modalStyles}>
                ...
                <Rating count={5} size={40} value={rating} onChange={handleRating} />
                <textarea className="w-full p-2 border-2 border-gray-300 rounded-md mt-2" value={newReview} onChange={(e) => setNewReview(e.target.value)} placeholder="Your review" rows={4}></textarea>
                <div className="flex justify-end mt-4">
                    <button className="px-4 py-2 bg-blue-500 text-white rounded-lg" onClick={handleReviewSubmit}>Submit Review</button>
                </div>
            </Modal>
            ...
            <Modal isOpen={photoModalIsOpen} onRequestClose={() => setPhotoModalIsOpen(false)} style={modalStyles}>
                ...
                <div className="flex justify-end mt-4">
                    <button className="px-4 py-2 bg-green-500 text-white rounded-lg" onClick={handlePhotoUpload} disabled={isUploading}>
            ...
            <Modal isOpen={loginModalIsOpen} onRequestClose={() => setLoginModalIsOpen(false)} style={modalStyles}>
                ...
                <div className="flex justify-end mt-4">
                    <button className="px-4 py-2 bg-blue-500 text-white rounded-lg" onClick={handleLogin}>Login with Google</button>
                </div>
            </Modal>
        </div>
    );
